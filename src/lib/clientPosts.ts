// Client-side posts management for static hosting
import { BlogPost } from './types';

// In a static hosting environment, we'll need to manage posts differently
// For now, we'll use localStorage for demo purposes, but in production 
// you might want to integrate with Firebase Firestore or another database
...
// Initialize posts in localStorage if not present
function initializePosts(): void {
    if (typeof window === 'undefined') return; // Server-side check

    const stored = localStorage.getItem(POSTS_STORAGE_KEY);
    if (!stored) {
        localStorage.setItem(POSTS_STORAGE_KEY, JSON.stringify(SAMPLE_POSTS));
    }
}

// Get all posts
export function getAllPosts(): BlogPost[] {
    if (typeof window === 'undefined') return SAMPLE_POSTS; // Server-side fallback

    initializePosts();
    const stored = localStorage.getItem(POSTS_STORAGE_KEY);
    if (!stored) return SAMPLE_POSTS;
...
export function savePost(post: BlogPost): void {
    ...
    // Update reading time and timestamp
